...
const ItineraryView: React.FC<ItineraryViewProps> = ({ days, setDays }) => {
  ...
  const getWeatherTheme = (cond: string) => {
    switch(cond) {
        case 'sunny': return {
            bg: 'bg-gradient-to-r from-orange-100 to-amber-100 border-orange-200/50',
            text: 'text-orange-900',
            iconColor: 'text-orange-500',
            labelBg: 'bg-white/40',
        };
        case 'cloudy': return {
            bg: 'bg-gradient-to-r from-blue-100 to-sky-100 border-blue-200/50',
            text: 'text-blue-900',
            iconColor: 'text-blue-500',
            labelBg: 'bg-white/40',
        };
        case 'rain': return {
            bg: 'bg-gradient-to-r from-gray-200 to-slate-200 border-gray-300/50',
            text: 'text-gray-800',
            iconColor: 'text-slate-500',
            labelBg: 'bg-white/40',
        };
        case 'storm': return {
            bg: 'bg-gradient-to-r from-slate-300 to-gray-400 border-slate-400/50',
            text: 'text-gray-900',
  ...
  return (
    <div className="flex flex-col h-full relative font-sans" ref={scrollContainerRef} onScroll={handleScroll} style={{overflowY: 'auto'}}>
      ...
      {/* Day Selector */}
      <div className={`sticky top-0 z-20 transition-all duration-500 ease-out ${isScrolled ? 'bg-[#F2F2F7]/90 backdrop-blur-xl border-b border-gray-200/50' : 'bg-transparent'}`}>
        <div 
          ref={dayListRef}
        ...
        >
            ...
            return (
                ...
                className={`flex-shrink-0 snap-center flex flex-col items-center justify-center transition-all duration-300 rounded-2xl border
                    ...
                    ${selectedDayId === day.id 
                        ? 'bg-[#007AFF] border-[#007AFF] text-white shadow-lg shadow-blue-200/50' 
          ...
          <button 
            ...
            className={`flex-shrink-0 snap-center flex items-center justify-center border border-dashed border-gray-300 text-gray-400 hover:border-blue-300 hover:text-blue-500 transition-all bg-white/40 rounded-2xl
      ...
      <div className="flex-1 p-5 pt-2 pb-[40vh]"> 
        ...
        <div className="flex flex-col gap-6 mb-8">
            <div className="flex justify-between items-end px-1">
                 <div>
                    <h2 className="text-4xl font-black text-black tracking-tighter">{currentDay.dayLabel}</h2>
        ...
        {currentDay.items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-gray-300 bg-white/30 backdrop-blur-sm rounded-[2rem] border-2 border-dashed border-gray-200/50 mt-4">
            <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mb-4">
                <i className="fa-solid fa-map-location-dot text-2xl opacity-40"></i>
            </div>
            <p className="font-bold text-sm text-gray-400">尚未安排行程</p>
            <p className="text-xs text-gray-300 mt-1">點擊右下角 + 開始規劃</p>
          </div>
        ) : (
            ...
            {currentDay.items.map((item, index) => {
              ...
              if (item.type === 'flight') {
                return (
                  ...
                  >
                     <div className="absolute left-1/2 -top-6 -translate-x-1/2 w-10 h-10 rounded-full bg-blue-500 text-white flex items-center justify-center shadow-lg shadow-blue-200 border-4 border-[#F2F2F7] z-20">
                        <i className={`fa-solid ${item.isArrival ? 'fa-plane-arrival' : 'fa-plane-departure'}`}></i>
                     </div>
                     <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-3xl text-white shadow-xl shadow-blue-100 overflow-hidden relative">
              ...
              return (
                ...
                >
                  ...
                  <div className="w-full bg-white rounded-3xl shadow-[0_20px_40px_-12px_rgba(0,0,0,0.1)] transition-shadow overflow-hidden border border-gray-100/50">
                    {item.imageUrl && (
                        <div className="h-48 w-full relative bg-gray-100">
                            ...
                            />
                            <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold shadow-sm flex items-center gap-2">
                    ...
                    <div className="p-6">
                        {!item.imageUrl && (
                             <div className="flex items-center justify-between mb-4">
                                 <h3 className="text-2xl font-bold text-gray-900 leading-tight">{item.location}</h3>
                                 <div className="bg-gray-100 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-2">
                                    <i className={`fa-solid ${getTypeIcon(item.type)}`}></i>
                                    <time className="font-mono">{item.time}</time>
                                 </div>
                             </div>
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-4">
                            {item.rating && (
                                <span className="flex items-center gap-1 text-[10px] font-bold text-orange-500 bg-orange-50 px-2 py-1 rounded-lg">
                                    <i className="fa-solid fa-star text-[9px]"></i> {item.rating}
                                </span>
                            )}
                            {(item.price || item.openTime) && (
                                <span className="text-[10px] text-gray-400 flex items-center gap-3 ml-auto font-medium">
                        ...
                        <div className="flex justify-end gap-2 mt-6 pt-4 border-t border-gray-50 opacity-0 group-hover:opacity-100 transition-opacity">
                             <button onClick={() => openEditModal(item)} className="w-8 h-8 rounded-full bg-gray-50 hover:bg-blue-50 text-gray-400 hover:text-blue-600 flex items-center justify-center transition-colors">
                                <i className="fa-solid fa-pen text-xs"></i>
                             </button>
                             <button onClick={() => handleDeleteItemClick(item.id)} className="w-8 h-8 rounded-full bg-gray-50 hover:bg-red-50 text-gray-400 hover:text-red-600 flex items-center justify-center transition-colors">
      ...
      <div className="fixed bottom-28 right-6 z-40">
        ...
        {fabActions.map((btn, idx) => {
            ...
            return (
                ...
                >
                     <span className={`absolute whitespace-nowrap bg-black text-white text-xs font-bold px-2 py-1 rounded-md opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none -top-8 shadow-lg`}>
                        {btn.label}
                     </span>

                     <button 
                        onClick={() => { btn.action(); setIsFabOpen(false); }} 
                        className={`w-12 h-12 rounded-full flex items-center justify-center shadow-lg border border-white/50 transition-all hover:scale-110 active:scale-95 bg-white text-blue-600
        ...
        <button 
            ...
            className={`relative z-50 w-16 h-16 rounded-full flex items-center justify-center text-2xl transition-all duration-300
                ...
                shadow-[0_4px_20px_rgba(0,0,0,0.1),inset_0_0_10px_rgba(255,255,255,0.5)]
                backdrop-blur-[2px]
                ${isFabOpen 
                ? 'text-white rotate-45 bg-black/80 border-transparent shadow-xl' 
                : 'text-blue-600 hover:scale-105 active:scale-95'
            }`}
        >
      ...
      {[
        { show: showAiModal, close: () => setShowAiModal(false), title: 'AI 智能排程', icon: 'fa-wand-magic-sparkles text-purple-500', content: (
            <>
                <textarea className="w-full p-4 bg-gray-50 rounded-2xl focus:bg-white focus:ring-2 focus:ring-purple-100 outline-none resize-none h-32 text-sm text-black" placeholder="我想去..." value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} />
                <button onClick={handleAiGenerate} disabled={isGenerating || !aiPrompt.trim()} className="w-full mt-6 py-3 rounded-xl font-bold text-white bg-black hover:bg-gray-800 transition-all">{isGenerating ? '生成中...' : '生成行程'}</button>
            </>
        )},
            ...
            <div className="space-y-4">
                {isManagingTypes ? (
                    <div className="space-y-2">
                        ...
                        {availableTypes.map(t => <div key={t.value} className="flex justify-between p-2 bg-gray-50 rounded"><span className="flex items-center gap-2"><i className={`fa-solid ${getTypeIcon(t.value)}`}></i>{t.label}</span> <button onClick={() => handleDeleteTypeClick(t.value)} className="text-red-400"><i className="fa-solid fa-trash"></i></button></div>)}
                        <div className="flex gap-2"><input value={newTypeName} onChange={e => setNewTypeName(e.target.value)} className="flex-1 bg-gray-100 p-2 rounded" placeholder="新類型" /><button onClick={handleAddType} className="bg-black text-white px-3 rounded font-bold">新增</button></div>
                    </div>
                ) : (
                    <>
                        <input type="text" placeholder="地點名稱" value={manualName} onChange={e => setManualName(e.target.value)} className="w-full p-3 bg-gray-50 rounded-2xl outline-none font-bold text-black" />
                        <div className="flex gap-3">
                            <input type="time" value={manualTime} onChange={e => setManualTime(e.target.value)} className="flex-1 p-3 bg-gray-50 rounded-xl outline-none font-bold" />
                            <div className="flex-1 relative">
                                <select value={manualType} onChange={e => setManualType(e.target.value)} className="w-full p-3 pl-9 bg-gray-50 rounded-xl outline-none appearance-none font-bold text-gray-700">{availableTypes.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}</select>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                             <input type="number" placeholder="評分" value={manualRating} onChange={e => setManualRating(e.target.value)} className="w-full p-3 bg-gray-50 rounded-xl outline-none text-sm" />
                             <input type="text" placeholder="營業時間" value={manualOpenTime} onChange={e => setManualOpenTime(e.target.value)} className="w-full p-3 bg-gray-50 rounded-xl outline-none text-sm" />
                        </div>
                        <input type="text" placeholder="圖片連結" value={manualImage} onChange={e => setManualImage(e.target.value)} className="w-full p-3 bg-gray-50 rounded-xl outline-none text-xs" />
                        <textarea value={manualDesc} onChange={e => setManualDesc(e.target.value)} rows={3} className="w-full p-3 bg-gray-50 rounded-xl outline-none text-sm" placeholder="備註..." />
                        <button onClick={handleSaveManual} disabled={!manualName} className="w-full mt-4 py-3 rounded-xl font-bold text-white bg-black hover:bg-gray-800 transition-all">確認</button>
                    </>
                )}
            </div>
        )},
        { show: showFlightModal, close: () => setShowFlightModal(false), title: '航班資訊', content: (
            <div className="space-y-3">
               ...
               <div className="flex bg-gray-100 p-1 rounded-2xl">
                  <button onClick={() => setFlightType('arrival')} className={`flex-1 py-2.5 text-sm font-bold rounded-xl transition-all ${flightType === 'arrival' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-400'}`}>去程</button>
                  <button onClick={() => setFlightType('departure')} className={`flex-1 py-2.5 text-sm font-bold rounded-xl transition-all ${flightType === 'departure' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-400'}`}>返程</button>
               </div>
               
               {/* Flight No */}
               <div>
                 <label className="block text-xs font-bold text-gray-400 mb-1 pl-1">航班號碼</label>
                 <input type="text" placeholder="e.g. CI501" value={flightNo} onChange={e => setFlightNo(e.target.value.toUpperCase())} className="w-full p-3 bg-gray-50 rounded-2xl font-mono font-bold text-lg uppercase outline-none focus:bg-blue-50 focus:ring-2 focus:ring-blue-100 transition-all" />
               </div>
               ...
               <div className="flex gap-2 items-start">
                 <div className="flex-1 min-w-0">
                    ...
                    <input type="text" placeholder="TPE" value={flightOrigin} onChange={e => setFlightOrigin(e.target.value.toUpperCase())} className="w-full p-3 bg-gray-50 rounded-2xl font-mono uppercase text-center font-black text-xl outline-none focus:bg-blue-50 transition-all mb-2" />
                    <input type="text" placeholder="航廈 T1" value={flightOriginTerminal} onChange={e => setFlightOriginTerminal(e.target.value.toUpperCase())} className="w-full p-2 bg-gray-50 rounded-xl font-mono uppercase text-center text-sm font-bold outline-none focus:bg-blue-50 transition-all" />
                 </div>
                 ...
                 <div className="flex-1 min-w-0">
                    ...
                    <input type="text" placeholder="PVG" value={flightDestination} onChange={e => setFlightDestination(e.target.value.toUpperCase())} className="w-full p-3 bg-gray-50 rounded-2xl font-mono uppercase text-center font-black text-xl outline-none focus:bg-blue-50 transition-all mb-2" />
                    <input type="text" placeholder="航廈 T2" value={flightDestTerminal} onChange={e => setFlightDestTerminal(e.target.value.toUpperCase())} className="w-full p-2 bg-gray-50 rounded-xl font-mono uppercase text-center text-sm font-bold outline-none focus:bg-blue-50 transition-all" />
                 </div>
               </div>
               ...
               <div className="flex gap-3">
                 <div className="flex-1 min-w-0">
                    ...
                    <input type="time" value={flightDepTime} onChange={e => setFlightDepTime(e.target.value)} className="w-full p-3 bg-gray-50 rounded-2xl font-bold outline-none focus:bg-blue-50 transition-all" />
                 </div>
                 <div className="flex-1 min-w-0">
                    <label className="block text-xs font-bold text-gray-400 mb-1 pl-1">抵達時間</label>
                    <input type="time" value={flightArrTime} onChange={e => setFlightArrTime(e.target.value)} className="w-full p-3 bg-gray-50 rounded-2xl font-bold outline-none focus:bg-blue-50 transition-all" />
                 </div>
               </div>
               
               <button onClick={handleSaveFlight} className="w-full mt-2 py-4 rounded-2xl font-bold text-white bg-[#007AFF] hover:bg-blue-600 shadow-xl shadow-blue-200 active:scale-95 transition-all">儲存航班</button>
            </div>
        )}
      ].map((m, i) => m.show && (
        <div key={i} className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl animate-scale-up">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-bold text-black flex items-center gap-2">{m.icon && <i className={`fa-solid ${m.icon}`}></i>} {m.title}</h3>
                    <button onClick={m.close} className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-gray-200"><i className="fa-solid fa-xmark"></i></button>
                </div>
                {m.content}
            </div>
        </div>
      ...
      {/* Confirmation Modal */}
      {deleteConfirmation && (
         <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/20 backdrop-blur-sm p-4 animate-fade-in">
