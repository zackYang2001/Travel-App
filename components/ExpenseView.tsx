...
const ExpenseView: React.FC<ExpenseViewProps> = ({ expenses, setExpenses, users }) => {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto pb-40 font-sans no-scrollbar">
      {/* Header Card */}
      <div className="mx-4 mt-2 bg-gradient-to-br from-[#00C6FF] to-[#0072FF] dark:from-[#0055aa] dark:to-[#003388] rounded-[2rem] p-6 shadow-2xl shadow-blue-200 dark:shadow-blue-900/30 text-white relative overflow-hidden shrink-0 z-10">
      ...
      <div className="px-4 space-y-6 mt-6 pb-32">
        {/* Balances Section */}
        <div className="bg-white dark:bg-slate-900 rounded-[1.5rem] p-5 shadow-sm border border-gray-100 dark:border-slate-800">
          <h3 className="text-gray-400 dark:text-gray-500 font-bold mb-4 text-xs uppercase tracking-wider flex items-center gap-2"><i className="fa-solid fa-scale-balanced"></i> 分帳狀態</h3>
              ...
              return (
                <div key={b.userId} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <img src={user?.avatar} alt={user?.name} className="w-10 h-10 rounded-full border border-gray-100 dark:border-slate-800 bg-gray-100 dark:bg-slate-800" />
        ...
        {/* Expenses List */}
        <div>
          <h3 className="text-gray-400 dark:text-gray-500 font-bold mb-3 text-xs uppercase tracking-wider px-2">最近消費</h3>
          <div className="space-y-3">
            {expenses.map(expense => {
              ...
              return (
                <div 
                    key={expense.id} 
                    className="relative group expense-item h-20 w-full bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-50 dark:border-slate-800 overflow-hidden"
                ...
                >
                  ...
                  <div className="absolute left-4 top-0 bottom-0 flex items-center gap-4 z-20 pointer-events-none">
                      <div className="w-10 h-10 rounded-full bg-gray-50 dark:bg-slate-800 text-gray-500 flex items-center justify-center shrink-0">
                          <i className="fa-solid fa-receipt"></i>
                      </div>
                      <div className="min-w-0 max-w-[140px] sm:max-w-[200px]">
                        <p className="font-bold text-black dark:text-white text-sm truncate">{expense.description}</p>
                  ...
                  <div className={`absolute right-4 top-0 bottom-0 flex flex-col justify-center text-right z-20 transition-transform duration-300 ease-out pointer-events-none ${isSwiped ? '-translate-x-28' : 'translate-x-0'}`}>
                      <p className="font-black text-black dark:text-white font-mono text-lg">${expense.amount}</p>
                      <p className="text-[10px] text-gray-300">{expense.date.slice(5)}</p>
                  </div>

                  {/* Actions Layer: Slide In from Right */}
                  <div className={`absolute top-0 right-0 bottom-0 w-28 bg-gray-100 dark:bg-slate-800 flex items-center justify-center gap-3 transition-transform duration-300 ease-out z-30 ${isSwiped ? 'translate-x-0' : 'translate-x-full'}`}>
                      <button 
                        onClick={(e) => { e.stopPropagation(); handleEditClick(expense); }}
                        className="w-10 h-10 rounded-full bg-white dark:bg-slate-700 text-gray-600 dark:text-gray-300 flex items-center justify-center shadow-sm active:scale-90 transition-transform hover:text-blue-600"
                      >
      ...
      {isAdding && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/30 dark:bg-black/50 backdrop-blur-sm animate-fade-in">
          <form onSubmit={handleSaveExpense} className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl w-full max-w-md rounded-t-[2rem] sm:rounded-3xl p-8 animate-slide-up shadow-2xl border border-white/50 dark:border-white/10">
            <h3 className="text-2xl font-black mb-6 text-black dark:text-white">{editingId ? '編輯消費' : '記一筆'}</h3>
            <div className="space-y-6">
              <div className="flex bg-gray-100 dark:bg-slate-800/50 p-1 rounded-2xl">
                 <button type="button" onClick={() => setCurrency('TWD')} className={`flex-1 py-3 text-xs font-bold rounded-xl transition-all ${currency === 'TWD' ? 'bg-white dark:bg-slate-700 shadow text-black dark:text-white' : 'text-gray-400'}`}>TWD</button>
                 <button type="button" onClick={() => setCurrency('CNY')} className={`flex-1 py-3 text-xs font-bold rounded-xl transition-all ${currency === 'CNY' ? 'bg-white dark:bg-slate-700 shadow text-black dark:text-white' : 'text-gray-400'}`}>CNY</button>
              </div>
              {currency === 'CNY' ? (
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-50 dark:bg-slate-800/50 p-3 rounded-2xl"><label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">人民幣</label><div className="flex items-baseline"><span className="text-lg text-gray-400 font-bold mr-1">¥</span><input type="number" value={inputForeignAmount} onChange={e => handleForeignAmountChange(e.target.value)} className="w-full text-2xl font-bold bg-transparent outline-none text-black dark:text-white font-mono" placeholder="0" autoFocus={!editingId} /></div></div>
                    <div className="bg-gray-50 dark:bg-slate-800/50 p-3 rounded-2xl"><label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">匯率</label><input type="number" step="0.01" value={exchangeRate} onChange={e => setExchangeRate(e.target.value)} className="w-full text-xl font-bold bg-transparent outline-none text-gray-600 dark:text-gray-300 font-mono" /></div>
                    <div className="col-span-2 bg-blue-50 dark:bg-blue-900/30 p-4 rounded-2xl flex justify-between items-center"><span className="text-xs text-blue-500 font-bold">折合台幣</span><span className="text-2xl font-mono font-black text-blue-600 dark:text-blue-400">${newAmount || 0}</span></div>
                </div>
              ) : (
                <div className="bg-gray-50 dark:bg-slate-800/50 p-4 rounded-2xl border border-transparent dark:border-white/5"><label className="block text-xs font-bold text-gray-400 uppercase mb-1">金額 (TWD)</label><div className="flex items-baseline"><span className="text-2xl text-gray-400 font-bold mr-2">$</span><input type="number" step="1" required value={newAmount} onChange={e => setNewAmount(e.target.value)} className="w-full text-4xl font-black bg-transparent outline-none text-black dark:text-white placeholder-gray-200 dark:placeholder-slate-700 font-mono" placeholder="0" autoFocus={!editingId} /></div></div>
              )}
              <div className="bg-gray-50 dark:bg-slate-800/50 p-4 rounded-2xl border border-transparent dark:border-white/5"><label className="block text-xs font-bold text-gray-400 uppercase mb-2">消費項目</label><input type="text" required value={newDesc} onChange={e => setNewDesc(e.target.value)} className="w-full bg-transparent outline-none text-lg font-bold text-black dark:text-white placeholder-gray-300 dark:placeholder-slate-600" placeholder="例如：晚餐..." /></div>
              <div>
                ...
                <div className="flex gap-3 overflow-x-auto no-scrollbar p-2">
                  {users.map(user => (
                    <button key={user.id} type="button" onClick={() => setPayerId(user.id)} className={`flex flex-col items-center gap-2 p-3 rounded-2xl border-2 transition-all w-20 shrink-0 ${payerId === user.id ? 'bg-blue-50 dark:bg-blue-900/30 border-blue-500 text-blue-700 dark:text-blue-400 shadow-md transform scale-105' : 'bg-white dark:bg-slate-800 border-transparent text-gray-400 opacity-60 grayscale'}`}><img src={user.avatar} className="w-8 h-8 rounded-full bg-gray-100" alt="" /><span className="text-[10px] font-bold truncate w-full text-center">{user.name}</span></button>
                  ))}
                </div>
              </div>
            </div>
            <div className="flex gap-3 mt-8">
               <button type="button" onClick={resetForm} className="flex-1 py-4 bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 rounded-2xl font-bold text-gray-500 dark:text-gray-400">取消</button>
              <button type="submit" className="flex-1 py-4 bg-black dark:bg-blue-600 rounded-2xl font-bold text-white shadow-xl hover:bg-gray-800 dark:hover:bg-blue-500">{editingId ? '更新' : '確認新增'}</button>
            </div>
          </form>
        </div>
      )}
      ...
      {deleteExpenseId && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/20 dark:bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
             <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-3xl p-6 shadow-2xl max-w-xs w-full text-center border border-white/50 dark:border-white/10 animate-scale-up">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 font-medium mb-6 leading-relaxed">此動作無法復原，您確定要刪除這筆消費嗎？</p>
